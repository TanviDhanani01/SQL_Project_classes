...
select count(team) as toatl_team from group_stage_team_stats;
...
select team,count(matches_played) as matchplayed from group_stage_team_stats group by team ;
...
select team,100.0*(wins/matches_played) as win_percent
from group_stage_team_stats;

#7.	Write an SQL query to show which team has maximum goals_scored and their count
select * from group_stage_team_stats;
select team,max(goals_scored) as max_goals from group_stage_team_stats group by team ;
...
select team,round(100.0*(draws/matches_played),2) as draw_percent from group_stage_team_stats;
...
select team ,100.0*(losses/matches_played) as losses_percent from group_stage_team_stats order by losses_percent asc;

#11.	Write an SQL query to show the average goal_difference 
select * from group_stage_team_stats;
select avg(goal_difference) from group_stage_team_stats ; # it shows me 0 but that is not right answer

select team,avg(goal_difference) VG_GD from group_stage_team_stats group by team ;

#12.	 Write an SQL query to show name of the team where points are 0 
select * from group_stage_team_stats where points=0;
...
select * from overall_wc_stats ;
select * from overall_wc_stats where players_used =(select max(players_used ) from overall_wc_stats );

#17.	Write an SQL query to show each team name and avg_age in ascending order by avg_age
select team,avg_age from overall_wc_stats order by avg_age desc;
...
select * from overall_wc_stats where games >= 5;
...
select team,goals_per90,assists_per90,goals_assists_per90 from overall_wc_stats order by goals_assists_pens_per90 desc;
...
select `group`,max(expected_goal_scored) as max_expected_gs from group_stage_team_stats group by `group` order by max_expected_gs  asc;

#38.	Write an SQL query to show group, minimum exp_goal_conceded by each group in descending order by exp_goal_conceded 
select `group`,min(exp_goal_conceded) as min_exp_gc from group_stage_team_stats group by `group` order by min_exp_gc desc;
...
select  `group`, avg( exp_goal_difference_per_90) as avg_exp_gdp90 from group_stage_team_stats group by `group` order by avg_exp_gdp90 asc;

#40.	Write an SQL query to show which team has equal number of goals_scored and goals_against 
select * from group_stage_team_stats where goals_scored =goals_against ;

#41.	Write an SQL query to show which team has maximum players_used 

select max(players_used) from overall_wc_stats;
select team ,max(players_used) from overall_wc_stats  group by team;
...
select gs.`group`,avg(ows.avg_age) as avg_a
from overall_wc_stats ows inner join group_stage_team_stats gs 
on ows.team =gs.team 
group by `group`
order by avg_a desc;
...
select gs.`group`,sum(ows.fouls) as sum_fouls from
overall_wc_stats ows inner join group_stage_team_stats gs 
on ows.team =gs.team 
group by `group`
...
select gs.`group`,sum(ows.games) as sum_games from 
overall_wc_stats ows inner join group_stage_team_stats gs
on ows.team=gs.team 
group by `group` 
...
select gs.`group`,sum(ows.players_used)  as sum_players_used from 
overall_wc_stats ows inner join group_stage_team_stats gs
on ows.team =gs.team 
group by `group` 
...
select gs.`group`,sum(ows.offsides) as sum_offsides from 
overall_wc_stats ows inner join group_stage_team_stats gs
on ows.team =gs.team 
group by `group` 
...
select gs.`group`,avg(ows.passes_pct) as avg_passes_pct from
overall_wc_stats ows inner join group_stage_team_stats gs
on ows.team =gs.team 
group by `group` 
...
select gs.`group` ,avg(ows.goals_per90) as avg_goals_p90
from overall_wc_stats ows inner join group_stage_team_stats gs
on ows.team =gs.team 
group by `group` 
